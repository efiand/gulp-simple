...
const cssTest = () => src(`src/less/**/*.less`)
	.pipe(stylelint({ reporters: [{ console: true, formatter: `string` }] }))
	.pipe(lintspaces(pkg.lintspaces))
	.pipe(lintspaces.reporter());
...
const js = () => src(`src/js/script.js`)
	...
	.pipe(require(`webpack-stream`)({
		mode: `production`,
		module: {
			rules: [{ test: /\.js$/, use: { loader: `babel-loader`, options: pkg.babel } }]
		},
...
const img = () => src(`source/img/**/*.{svg,png,jpg}`)
	.pipe(imagemin([
		...
		imagemin.optipng(),
		require(`imagemin-jpegoptim`)({ max: 80, progressive: true })
	]))
	.pipe(dest(`build/img`))
...
const copy = () => src(`src/as-is/**/*.*`).pipe(dest(`build`));
